...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex justify-between items-center bg-[#FBEBB5] py-4 px-6 md:px-10 fixed top-0 z-50 shadow-md">
      {/* Logo */}
      <div>
      ...
      <div className="hidden sm:flex items-center gap-4">
        {[FaRegUser, FiSearch, FaRegHeart, AiOutlineShoppingCart].map((Icon, index) => (
          <Icon
            key={index}
            className="text-black text-xl cursor-pointer hover:text-[#000000ac] transition-transform transform hover:scale-110"
          />
        ))}
      </div>
    </nav>
